...
def read_cam():
    ...
    if cap.isOpened():
        ...
        helpText="'Esc' Salir, '1' Camara, '2' Filtro Canny , '3' Todos. '4' Esconder ayuda"
        edgeThreshold=40
        ...
        while True:
            ...
            if key == 27: # Check for ESC key
                ...
                cv2.setWindowTitle(windowName,"Camara")
                showWindow=1
            elif key==50: # 2 key, show Canny
                cv2.setWindowTitle(windowName,"Filtro Canny")
                showWindow=2
            elif key==51: # 3 key, show Stages
                cv2.setWindowTitle(windowName,"Camara, Escala de grises, Gaussian Blur, Canny")
                showWindow=3
            elif key==52: # 4 key, toggle help
                showHelp = not showHelp
            elif key==113: # Barra espaciadora lower canny edge threshold
                edgeThreshold=max(0,edgeThreshold-1)
                print 'Canny Edge Threshold Maximum: ',edgeThreshold
            elif key==32: # , raise canny edge threshold
